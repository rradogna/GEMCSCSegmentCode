...
MuonGEMSeedFromRecHits::MuonGEMSeedFromRecHits()
: MuonSeedFromRecHits()
{
}


TrajectorySeed MuonGEMSeedFromRecHits::seed() const
{
...
bool MuonGEMSeedFromRecHits::makeSeed(const MuonRecHitContainer & hits1, const MuonRecHitContainer & hits2,
                                      TrajectorySeed & seed) const
{
  for ( MuonRecHitContainer::const_iterator itr1 = hits1.begin(), end1 = hits1.end();
        itr1 != end1; ++itr1)
  {
    GEMDetId gemId1((*itr1)->geographicalId().rawId());
          ...
          itr2 != end2; ++itr2)
    {
      ...
      GEMDetId gemId2((*itr2)->geographicalId().rawId());
...
int MuonGEMSeedFromRecHits::segmentQuality(ConstMuonRecHitPointer  segment) const
{
  int Nchi2 = 0;
  int quality = 0;
  ...
  if(segment->isGEM() && GEMDetId(segment->geographicalId()).ring() == 4) ++quality;
  return quality;
}



MuonGEMSeedFromRecHits::ConstMuonRecHitPointer
MuonGEMSeedFromRecHits::bestEndcapHit(const MuonRecHitContainer & endcapHits) const
{
...
void MuonGEMSeedFromRecHits::makeDefaultSeed(TrajectorySeed & seed) const
{
  //Search ME1  ...
...
bool 
MuonGEMSeedFromRecHits::createDefaultEndcapSeed(ConstMuonRecHitPointer last, 
				 TrajectorySeed & seed) const {
...
void MuonGEMSeedFromRecHits::analyze() const 
{
  for ( MuonRecHitContainer::const_iterator iter = theRhits.begin(); iter!= theRhits.end(); iter++ ){
    if ( !(*iter)->isGEM() ) continue;
    for ( MuonRecHitContainer::const_iterator iter2 = iter + 1; iter2 != theRhits.end(); ++iter2)
    {
      if( !(*iter2)->isGEM() ) continue;

      GEMDetId gemId1((*iter)->geographicalId().rawId());
      GEMDetId gemId2((*iter2)->geographicalId().rawId());
      double dphi = deltaPhi((**iter).globalPosition().phi(), (**iter2).globalPosition().phi());
      std::cout << "HITPAIR," << "," <<dphi << "," << (**iter).globalPosition().eta() << std::endl;

      /*int type1 = gemId1.iChamberType();
      int type2 = gemId2.iChamberType();
      ...
      if(type1 == type2)
      {
        std::cout << "HITPAIRSAMESTATION," << type1 << gemId1.ring() << gemId2.ring()
           << "," << dphi << "," << (**iter).globalPosition().eta() << std::endl;
      }*/
